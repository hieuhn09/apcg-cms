...
**Last updated:** 2026-09-11 (UPDATE PROCESS pass — no source edits this pass; Fix 2b reconciled as shipped + production-verified)
...
| Phase 2 item — Fix 2b (`LIST_SELECT` field-drop) | ✅ **DONE, production-verified.** Merged to `main` at `4917ef5` (branch `fix/list-select-drop-internal-fields`, `45a02b8`; production build `apcg-kmasetrjs`, Ready 11/09 15:54 GMT+7). `LIST_SELECT` in `src/app/api/public/articles/route.ts` now also excludes `tenant`, `translationStatus`, `lastEngine`, `lastEditedBy`, `assignedTo` (+21 lines incl. a rationale/contract-lock comment). EVL iter 7 HALTED_SUCCESS: Part 1 (vc-tester) 10/10 PASS incl. field-name sanity; Part 2 (orchestrator, preview `apcg-5zu9z7i9t`) 58→53 keys/doc, five fields absent, pin keys intact, no unexpected loss, identity 107,403→78,663 B (−26.8%), gzip wire 12,643→10,986 B. Production post-deploy: 53 keys/doc, 78,633 B; health probes OK on brief-asia (9841, 206 ms), WAD (9839, 165 ms), DTW (9804, 173 ms), GCV (9836, 164 ms); WTB homepage 200 with hero rendered. Cost value post-gzip is only ~$1.34/mo — shipped for data-hygiene/PII-closure, not cost. |
| Phase 2 items — Fix #1 (fan-out collapse), Fix #3 (search), crawler-control | 🚧 **BLOCKED** on P1/P2b (Phase 0 not fully met). Fix #3 (search) needs only P1's `q=` share to start, per the Phase 2 branch logic — check whether P1 alone unblocks it before waiting on P2b too. |
| Phase 3 (R2 cutover) | 🚧 **BLOCKED.** P8a-c not run (P8d alone, GCV, does not clear Phase 3); no R2 domain attached; `R2_PUBLIC_BASE_URL` not flipped. |
...
| E10 (A3 body wording) | ✅ **RESOLVED** (10-09-26 pass) — see `## Additional Contract Locks` row A3. |
...
| Task-folder debt | Two cloud-agent PRs (#11, #12, branch `claude/tender-ptolemy-njgwwt`) landed on `main` mid-loop; contract locks (E1/A3/A4/depth:0) survived and were re-verified, but by luck, not by a guard. Fix 2b's own EXECUTE partially closed this: the `LIST_SELECT` comment now carries A3/A4 reminders, but the `refsView` `select` block still has none — see `## Follow-up: Guard Comments for Contract Locks` below. |
| E13 (gzip Observability confirmation) | 🚧 **STILL UNREAD.** gzip live since 2026-09-10 16:47. Read Usage → Fast Origin Transfer → project apcg-cms → last 48h; expect a 6-8x step-down at that hour. Revert path if it did not drop: `git revert 0ecc6bb`. |

**Next action for a fresh session:** read E13 first (see row above — time-sensitive, one-commit revert if it fails). Then Phase 0's P1/P2b. Then Fix #1 (gated on both) or Fix #3 (gated on P1's `q=` share alone — check if that alone clears it). Fix 2b is DONE — do not re-ship it. Phase 3 remains blocked (P8a-c not run; P8d alone does not clear Phase 3).
...
On the $96.48 baseline: gzip alone → ~$10.14; gzip + field-drop → ~$8.80. **Fix 2b's cost value after gzip is ~$1.34/month.** Its remaining justification is data hygiene (not emitting internal fields on a public API) and the PII/secret leak closure (see `## Security Finding`) — those stand on their own; do not over-prioritise 2b on cost grounds going forward.

**SHIPPED 2026-09-11** (`4917ef5`, production build `apcg-kmasetrjs`) — the pre-ship estimate above matched measurement almost exactly: predicted 26.8% raw / 13.1% gzipped; measured on real production docs 58→53 keys/doc, identity 107,403→78,663 B (**−26.8%**), gzip wire 12,643→10,986 B (**−13.1%**). See `## Current Execution State` for the full production verification detail.
...
## Follow-up: Guard Comments for Contract Locks (found 10-09-26, PARTIALLY DONE 2026-09-11 — `refsView` still open)

Two cloud-agent PRs (#11, #12, branch `claude/tender-ptolemy-njgwwt`) landed on `main` mid-loop during this plan's PVL/EVL cycles, each carrying its own migration (already applied via their own deploys). PR #11 extended `LIST_SELECT` in `articles/route.ts` (four `video*: false` keys) and added a `video` spread on the `[slug]` route. E1/A3/A4/`depth:0` were re-verified intact on the merged tree — but that survival was luck: the other agent had no way to know these contract locks existed, since nothing in `articles/route.ts` named them.

**PARTIALLY DONE 2026-09-11 (Fix 2b EXECUTE, `4917ef5`):** the `LIST_SELECT` extension in `src/app/api/public/articles/route.ts` shipped with a rationale/contract-lock comment above it — this closes the `LIST_SELECT` half of the follow-up.

**Still open (code change, schedule for the next EXECUTE touching this file):** the `refsView` `select` object has NO comment naming A3 (refsView select must never include `title`) or A4 (`pinnedToLatest`/`pinnedUntil` must remain list keys). Add one the next time this file is touched for any reason — a future agent editing `refsView` in isolation still has nothing pointing it at the invariant.
...
1. **Selected plan file path:** `process/general-plans/active/cms-cost-remediation_09-09-26/cms-cost-remediation_PLAN_09-09-26.md` (this file). Task folder also holds `results.tsv` (PVL/EVL iterations 0-7), two PVL iteration reports, the Phase 1 EXECUTE report (`cms-cost-remediation_REPORT_10-09-26.md`), two EVL notes (`cms-cost-remediation-evl-gzip_NOTE_10-09-26.md`, `cms-cost-remediation-evl-fix2b_NOTE_11-09-26.md`), the gzip feasibility artifact (`gzip-passthrough_FEASIBILITY_10-09-26.md`), and the closeout packet (`cms-cost-remediation_CLOSEOUT_10-09-26.md`, refreshed 2026-09-11).
2. **Last completed phase or step (as of 2026-09-11 UPDATE PROCESS pass):** Four production changes now shipped — Phase 1 (`0ae9fa3`), gzip (`0ecc6bb`), GCV audit (`ef6b5e5`, no code, cleared the last Fix 2b blocker), and Fix 2b (`4917ef5`). See `## Current Execution State` above for the authoritative per-phase status table — read that first, it supersedes the historical PLAN-mode summary below.
3. **Validate-contract status:** present, PVL cycle 3 (`Gate: CONDITIONAL`, `generated-by: outer-pvl`, dated 2026-09-10) — see `## Validate Contract`. EVL confirmed all gates green for Phase 1, gzip, and Fix 2b (`results.tsv` iterations 3, 6, and 7, all `HALTED_SUCCESS`).
...
5. **Next step for a fresh agent picking up mid-execution — check in this order:**
   - **First: E13, still unread.** Read Vercel Observability's per-route bytes for `/api/public/articles` (Usage → Fast Origin Transfer → project apcg-cms → last 48h; expect a 6-8x step-down at the 2026-09-10 16:47 deploy hour). If it did NOT drop as predicted: `git revert 0ecc6bb` (one commit, no data impact) — do not attempt to "fix forward" the metering assumption first.
   - **Then: Phase 0's remaining two measurements, P1 and P2b** (query-string facet and full-August-window User-Agent facet on `/api/public/articles` in Vercel Logs). Both are Agent-Probe/dashboard-read tier. Fix #1 needs both; Fix #3 (search) needs only P1's `q=` share — check whether P1 alone is enough to unblock it.
   ...
   - **Fix 2b is DONE — do not re-ship it.** `LIST_SELECT` field-drop shipped and is production-verified (`4917ef5`). One follow-up remains from its EXECUTE: the `LIST_SELECT` comment now names A3/A4, but the `refsView` `select` block still has no comment — add one the next time this file is touched (see `## Follow-up: Guard Comments for Contract Locks`).
   - **Optional, low-effort, not gating anything:** audit `brief-asia`/`wtb`/`wad` tenants' `frontendUrl` values for the same rot found on `dtw` (see `## Defect Found: DTW frontendUrl...`).
   ...
   - Fix 2a (`defaultPopulate` security fix) is DONE and production-verified — do not redo it.
   - Before touching Fix #4, confirm P8a-d have all passed. **P8d is CLEAR for all 5 readers (GCV cleared 2026-09-11).** P8a-c (R2 domain/derivative/prefix checks) are still NOT RUN and remain the actual Phase 3 blocker. Deploy and verify the redirect shim BEFORE flipping the env var — do not reverse this order.
   ...
   - Two cloud-agent PRs (#11, #12) landed on `main` mid-loop and were re-verified non-interfering with E1/A3/A4/depth:0 — the `LIST_SELECT` half of the durable guard-comment fix shipped with Fix 2b; the `refsView` half is still open (see above).
   - Cleanup, not gating anything: delete merged remote branches `probe/gzip-public-api` and `fix/list-select-drop-internal-fields`; user to remove `CMS_READ_TOKEN` from `.env.local` and regenerate the Vercel bypass secret.
